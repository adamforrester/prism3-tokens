...
| 8 ✅ | shadow/gradient | **Done.** Write scope reaches Figma **Styles** — shadow → Effect Styles (`shadow/*` + `shadow-dark/*`), gradient → Paint Styles (baked resolved stops + angle→`gradientTransform`). Node-free `emit-figma-styles.ts` + pure `buildStylesPlan`/`gradientTransformFor` + `applyStylesPlan` (new `StylesApi` port, the first non-variable write). Only typography remains (own lane, #112/#113). |
