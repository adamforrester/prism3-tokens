...
**One increment still open:** optional **per-step hue drift** to reproduce brand hand-kinks where a brand wants them (NB `amber.600`, `red.300` — the only remaining ΔE00 outliers). Constant-hue is the right default; this would be opt-in.
