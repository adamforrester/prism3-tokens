...
- `standard-design-md.ts` / `classify-colors.ts` / `spike-wendys.ts` — the **E2E integration spike** (docs/07 §11.6, additive; does not touch the shipped step-A pipeline). `standard-design-md.ts` reads the STANDARD brand-skills/google-labs `design.md` (flat `colors` hex map + structured type/dimension maps — a different shape from `design-md.ts`, which reads the engine's own `BrandInput` frontmatter). `classify-colors.ts` is the colour-role classifier: flat `colors:` → engine anchors by naming convention (`primary`→pinned, `secondary`/`tertiary`→brand palettes, `neutral-<step>`→derived hue/chroma, `success`/`warning`/`error`→`danger`→status; `info`/`white`/variants report-only). `spike-wendys.ts` runs a real Wendy's `design.md` (`../examples/wendys.design.md`) through the shipped core and writes `out/wendys.*` + a full-parity fidelity report (`wendys-fidelity-report.md`). Run: `npx tsx Prism3/engine/spike-wendys.ts`.
